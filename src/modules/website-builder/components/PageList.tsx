...
import { Plus, Home, Copy, Trash2, Edit } from 'lucide-react'
...
interface PageListProps {
  ...
  /** Optional: not used here; cloning is delegated to parent */
  siteId?: string
}
...
export default function PageList({
  ...
  const handleCreatePage = () => {
    ...
    onCreatePage({ name: newPageForm.name, slug, template: newPageForm.template })
  ...
  const handleEditPage = () => {
    ...
    onEditPage(editingPage.id, { name: editingPage.name, slug: editingPage.slug })
    setEditingPage(null)
  }
    ...
    <Card>
      <CardHeader>
        <div className="flex items-center justify-between">
          ...
          <Button size="sm" onClick={() => setShowNewPageModal(true)}>
        ...
        {pages.map((page) => (
          <div
            ...
            className={cn(
              ...
              currentPageId === page.id ? 'bg-primary/10 border-primary' : 'hover:bg-accent'
            )}
            onClick={() => onSelectPage(page.id)}
          >
            <div className="flex items-center gap-2 flex-1 min-w-0">
              {page.isHomePage && <Home className="h-4 w-4 text-primary flex-shrink-0" />}
            ...
            <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
              ...
              <Button
                ...
                onClick={(e) => {
                  e.stopPropagation()
                  onDuplicatePage(page.id) // delegate cloning to parent
                }}
                className="h-7 w-7 p-0"
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <Input
              ...
              onChange={(e) =>
                setNewPageForm({ ...newPageForm, slug: e.target.value.toLowerCase() })
              }
              placeholder="e.g., about-us"
            />
            <p className="text-xs text-muted-foreground mt-1">
              This will be the page URL: /{newPageForm.slug || 'your-slug'}
            </p>
          </div>
