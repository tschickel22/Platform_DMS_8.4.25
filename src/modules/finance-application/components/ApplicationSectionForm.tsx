...
export default function AccountDetail() {
  ...
  const handleApplicationSaveDraft = (patch: any) => {
    const existing = loadFromLocalStorage<any[]>('applications', [])
    const draft = { id: generateId(), accountId, templateId: 'quick-fallback', status: 'draft', ...patch }
    saveToLocalStorage('applications', [draft, ...existing])
    refreshSection('applications')
    toast({ title: 'Draft Saved', description: 'Application draft saved successfully' })
  }
  const handleApplicationSubmit = (patch: any) => {
    const existing = loadFromLocalStorage<any[]>('applications', [])
    const submitted = { id: generateId(), accountId, templateId: 'quick-fallback', status: 'submitted', ...patch }
    saveToLocalStorage('applications', [submitted, ...existing])
    refreshSection('applications')
  ...
  const routeCreateForType = (t: SectionType) => {
    const map: Partial<Record<SectionType, string>> = {
      ...
      applications: undefined, // handled by modal
      invoices: `/invoices/new?accountId=${accountId}&returnTo=account`, // also have a modal
    }
    const href = map[t]
  ...
  // Build a fresh application object for the modal
  const newApplication = useMemo(() => ({
    id: generateId(),
    accountId,
    templateId: 'quick-fallback',
    status: 'draft',
    data: {},
    ...
    <>
      ...
      <Dialog open={openApplication} onOpenChange={setOpenApplication}>
        <DialogContent className="sm:max-w-4xl w-[95vw] max-h-[85vh] overflow-y-auto">
          <DialogTitle className="sr-only">Record Application</DialogTitle>
          ...
          <FinanceApplicationForm
            ...
            onSave={handleApplicationSaveDraft}
