...
export default function ContactList() {
  const { contacts, loading, metrics, createContact } = useContactManagement()
  ...
  const contactImportSchema = [
    ...
    { key: 'accountId', label: 'Account ID', required: false, type: 'string' },
    { key: 'tags', label: 'Tags (comma-separated)', required: false, type: 'string' },
  ]

  const handleImportContacts = (importedData: any[]) => {
    ...
    toast({ title: 'Import Complete', description: `${importedData.length} contacts imported.` })
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <ImportModal<Contact>
        ...
        sampleCsvUrl="/sample-contacts.csv"
      />
    </div>
  )
}